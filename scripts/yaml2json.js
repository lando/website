...
if (_.isEmpty(inputFiles)) {
  console.error('You must pass in at least one filename argument!');
  process.exit(2);
}
...
const inputFilePaths = _(inputFiles)
  ...
  .filter(file => {
    if (!fs.existsSync(file)) console.log('Could not locate file %s! Skipping that one...', file);
...
if (!fs.existsSync(outputDir)) {
  console.log('%s does not exists, creating it...', outputDir);
